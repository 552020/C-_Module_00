...
void Account::_displayTimestamp() {
  ...
  std::cout << '[' << std::setfill('0') << std::setw(4)
            << (localTime->tm_year + 1900) << std::setw(2)
...
void Account::displayAccountsInfos() {
  ...
  // Loop through each Account object and call displayStatus
  for (std::vector<Account *>::iterator it = Account::_accounts.begin();
       it != Account::_accounts.end(); ++it) {
    (*it)->displayStatus();
  }
}
